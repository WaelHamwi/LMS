    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="form-horizontal">
                @if ($student->images && count($student->images) > 0)
                @foreach ($student->images as $image)
                <input type="text" class="form-control mb-4" id="input-copy-{{ $student->id }}-{{ $loop->index }}"
                    value="{{ asset('attachments/students/' . $student->name . '/' . $image->filename) }}">
                <a class="mb-1 btn clip-btn btn-primary" href="javascript:;" data-clipboard-action="copy"
                    data-clipboard-target="#input-copy-{{ $student->id }}-{{ $loop->index }}">
                ...
                @endforeach
                @else
                <p class="text-danger">No images available for this student. Please attach images.</p>
                @endif
            </form>
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">Student attachements</h5>
                    </div>
                    <div class="card-body">
                                ...
                                <tbody>
                                    ...
                                    <div class="mb-3 col-4">
                                        ...
                                        <form action="{{ route('images.store') }}" method="POST" enctype="multipart/form-data">
                                            ...
                                            <input type="hidden" name="imageable_type" value="App\Models\Student">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <!-- Download Button -->
                                            <a href="{{ route('image.download', $image->id) }}" class="btn btn-outline-primary me-2">
                                            ...
                                            <button class="btn btn-outline-secondary me-2" data-bs-toggle="modal" data-bs-target="#imageViewerModal"
                                                data-image-url="{{ asset('attachments/students/' . $student->name . '/' . $image->filename) }}"
                                            ...
                                            @include('components.small-modal', [
                                            'target' => 'target2',
                                    ...
                                    @include('components.image-viewer', ['imageUrl' => asset('attachments/students/' . $student->name . '/' . $image->filename)])
                                    @endforeach
                                    @else
