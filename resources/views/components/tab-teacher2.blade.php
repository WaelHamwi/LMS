    ...
    <div class="card">
        ...
        <div class="card-body">
            <form class="form-horizontal">
                @if ($teacher->images && count($teacher->images) > 0)
                @foreach ($teacher->images as $image)
                <input type="text" class="form-control mb-4" id="input-copy-{{ $teacher->id }}-{{ $loop->index }}"
                    value="{{ asset('attachments/teachers/' . $teacher->first_name . '/' . $image->filename) }}">
                <a class="mb-1 btn clip-btn btn-primary" href="javascript:;" data-clipboard-action="copy"
                    data-clipboard-target="#input-copy-{{ $teacher->id }}-{{ $loop->index }}">
                ...
                @endforeach
                @else
                <p class="text-danger">No images available for this teacher. Please attach images.</p>
                @endif
            </form>
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">teacher attachements</h5>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table table-hover table-bordered mb-0">
                                ...
                                <tbody>
                                    ...
                                    <div class="mb-3 col-4">
                                        ...
                                        <form action="{{ route('images.storeTeacherImage') }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            <input type="file" name="images[]" id="image" class="form-control" multiple required>
                                            <input type="hidden" name="imageable_id" value="{{ $teacher->id }}">
                                            <input type="hidden" name="imageable_type" value="App\Models\Teacher">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <!-- Download Button -->
                                            <a href="{{ route('image.downloadTeacherImage', $image->id) }}" class="btn btn-outline-primary me-2">
                                            ...
                                            <button class="btn btn-outline-secondary me-2" data-bs-toggle="modal" data-bs-target="#imageViewerModal"
                                                data-image-url="{{ asset('attachments/teachers/' . $teacher->first_name . '/' . $image->filename) }}"
                                    ...
                                    @include('components.image-viewer', ['imageUrl' => asset('attachments/teachers/' . $teacher->first_name . '/' . $image->filename)])
                                    @endforeach
                                    @else
